...
class CrossFittedConditionalAIPW:
    """Cross-fit natural-history D by episode, then regress scores on X.

    Important semantic contract: this estimator targets the natural conditional
    history law P(W|X) because its second stage estimates E[phi|X].  It must
    not be silently reused for an arbitrary frozen/transported nu*.  Those
    targets require explicit standardization/transport outside this class.
    """

    def __init__(self, action_dim, n_folds=5, ridge=1e-3, iw_clip=None, history_target="natural_history"):
        if str(history_target) != "natural_history":
            raise ValueError(
                "CrossFittedConditionalAIPW targets natural_history=P(W|X); "
                "frozen_standardized/transported nu* needs explicit standardization or transport"
            )
        self.history_target = "natural_history"
    ...
    def fit(self, samples):
        ...
        self.diagnostics = {
            ...
            "crossfit_unit": "episode",
            "history_target": self.history_target,
            "target_history_law": "P(W|X)",
            "fold_assignment": {str(k): int(v) for k, v in fold_by_episode.items()},
            "fold_assignment_hash": hashlib.sha256(
                json.dumps(
                    {str(k): int(v) for k, v in fold_by_episode.items()},
                    sort_keys=True,
                    separators=(",", ":"),
                ).encode("utf-8")
            ).hexdigest(),
