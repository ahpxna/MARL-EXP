...
from research_chains.h1_fixed_panel import load_fixed_panel
...
def _push_proxy_replay_compat(runner, trajectory):
    """Push proxy replay through the runner's canonical episode-aware path.

    Scientific reason: CrossFittedConditionalAIPW splits by ``episode_id``.
    Calling ``replay_builder.push_trajectory_to_proxy`` directly bypasses the
    runner boundary that stamps ``episode_id``/``timestep`` and silently turns
    a many-episode development run into one episode from the estimator's point
    of view.  Prefer the canonical public method and only use a fail-closed
    annotated fallback for older runners.
    """
    if hasattr(runner, "push_trajectory_to_proxy_buffer"):
        return runner.push_trajectory_to_proxy_buffer(trajectory)
    ...
    if (
        ...
        episode_id = int(getattr(runner, "episodes_completed", 0))
        for timestep, step in enumerate(trajectory):
            if not isinstance(step, dict):
                raise TypeError("proxy replay trajectory steps must be dictionaries")
            step.setdefault("episode_id", episode_id)
    ...
    raise AttributeError(
        "Runner has neither push_trajectory_to_proxy_buffer(), "
        "push_proxy_replay(), nor replay_builder/proxy/env. Cannot push "
        "trajectory into local counterfactual proxy replay."
    )


def _pair_replay_support_diagnostics(
    buffer,
    action_dim,
    *,
    min_action_count=2,
    min_action_ess=2.0,
    min_behaviour_propensity=1e-6,
):
    """Pair-aggregated *data* support diagnostics from typed replay rows.

    This is deliberately different from target-policy entropy.  For each
    (ego, neighbour) relation it reports executed-action counts over actions
    that were valid in at least one stored row, behaviour-propensity support,
    and a Kish effective sample size based on inverse-propensity weights.

    It is not a state-conditional positivity theorem; ``support_scope`` in the
    returned object says exactly what is measured so later papers cannot
    silently reinterpret this diagnostic as local-state support.
    """
    rows = [row for row in list(buffer) if isinstance(row, dict)]
    groups = {}
    for row in rows:
        if row.get("ego_id") is None or row.get("neighbor_id") is None:
            continue
        key = (int(row["ego_id"]), int(row["neighbor_id"]))
        groups.setdefault(key, []).append(row)

    pair_rows = []
    poor_pairs = set()
    for (ego, neighbour), group in sorted(groups.items()):
        required = np.zeros(int(action_dim), dtype=bool)
        counts = np.zeros(int(action_dim), dtype=int)
        propensities = []
        weights_by_action = [[] for _ in range(int(action_dim))]
        episode_ids = set()
        for row in group:
            mask = np.asarray(
                row.get("valid_action_mask", np.ones(int(action_dim))), dtype=bool
            ).reshape(-1)
            if mask.shape != (int(action_dim),):
                raise ValueError("replay valid_action_mask has wrong shape")
            required |= mask
            action = int(row.get("target_action_executed", row.get("observed_action_j", -1)))
            if 0 <= action < int(action_dim) and bool(mask[action]):
                counts[action] += 1
                b = row.get("behaviour_prob_j")
                if b is not None and np.isfinite(float(b)) and float(b) > 0.0:
                    b = float(b)
                    propensities.append(b)
                    weights_by_action[action].append(1.0 / b)
            if row.get("episode_id") is not None:
                episode_ids.add(int(row["episode_id"]))

        required_actions = np.flatnonzero(required)
        supported = [int(a) for a in required_actions if counts[a] >= int(min_action_count)]
        coverage = (
            float(len(supported) / len(required_actions)) if len(required_actions) else float("nan")
        )
        action_ess = []
        for a in required_actions:
            w = np.asarray(weights_by_action[int(a)], dtype=np.float64)
            if w.size == 0:
                action_ess.append(0.0)
            else:
                denom = float(np.sum(w * w))
                action_ess.append(float((np.sum(w) ** 2) / denom) if denom > 0.0 else 0.0)
        min_ess = float(min(action_ess)) if action_ess else float("nan")
        min_prop = float(min(propensities)) if propensities else float("nan")
        propensity_available = bool(propensities)
        ess_available = bool(action_ess) and propensity_available
        poor = bool(
            len(required_actions) == 0
            or coverage < 1.0 - 1e-12
            or (ess_available and min_ess < float(min_action_ess))
            or (propensity_available and min_prop < float(min_behaviour_propensity))
        )
        if poor:
            poor_pairs.add((ego, neighbour))
        pair_rows.append({
            "ego_id": ego,
            "neighbor_id": neighbour,
            "n_rows": int(len(group)),
            "n_episode_ids": int(len(episode_ids)),
            "required_actions": [int(a) for a in required_actions],
            "action_counts": [int(x) for x in counts],
            "n_required_actions": int(len(required_actions)),
            "n_supported_actions": int(len(supported)),
            "action_coverage": coverage,
            "min_behaviour_propensity": min_prop,
            "min_action_ess": min_ess,
            "propensity_available": propensity_available,
            "ess_available": ess_available,
            "pair_data_support_poor": poor,
        })

    return {
        "support_scope": "pair_aggregated_replay_over_typed_rows_not_state_conditional",
        "min_action_count": int(min_action_count),
        "min_action_ess": float(min_action_ess),
        "min_behaviour_propensity": float(min_behaviour_propensity),
        "n_pairs": int(len(pair_rows)),
        "poor_pair_count": int(len(poor_pairs)),
        "pairs": pair_rows,
...
def _train_tiny_runner_for_proxy(tiny_env, tiny_cfg, args, device):
    ...
    for ep in range(train_episodes):
        ...
        # Mirror the canonical runner lifecycle.  The tiny calibration loop
        # intentionally does not call runner.run(), so it must advance the
        # global episode counter itself after the replay for this episode has
        # been stamped with the previous completed-episode id.
        runner.episodes_completed = int(getattr(runner, "episodes_completed", 0)) + 1
...
def _evaluate_h1_exact_protocol(runner, tiny_env, args, tiny_cfg):
    ...
    fixed_panel_path = getattr(args, "h1_fixed_panel_path", None)
    fixed_panel_payload = None
    if fixed_panel_path:
        fixed_panel_payload = load_fixed_panel(fixed_panel_path)
        eval_steps = fixed_panel_payload["steps"]
        if len(eval_steps) != int(args.tiny_states):
            raise RuntimeError(
                "Fixed H1 panel size does not match --tiny-states: "
                f"panel={len(eval_steps)} requested={int(args.tiny_states)}"
            )
        policy_return_metadata = {
            "heldout_policy_return_mean_per_agent": float("nan"),
            "heldout_policy_return_std_per_agent": float("nan"),
            "heldout_policy_return_n_episodes": 0,
            "policy_return_endpoint_measured": False,
            "fixed_eval_panel_mode": True,
            "fixed_eval_panel_protocol": fixed_panel_payload["protocol_version"],
            "fixed_eval_panel_fingerprint": fixed_panel_payload["panel_fingerprint"],
            "fixed_eval_panel_path": str(fixed_panel_path),
        }
    else:
        eval_steps, policy_return_metadata = _collect_h1_eval_steps(
            runner, int(args.tiny_states)
        )
        policy_return_metadata.update({
            "fixed_eval_panel_mode": False,
            "fixed_eval_panel_protocol": None,
            "fixed_eval_panel_fingerprint": None,
            "fixed_eval_panel_path": None,
        })
    crossfit = None
    crossfit_error = ""
    try:
    ...
    for state_idx, step in enumerate(eval_steps):
        panel_step_hash = step.get("_fixed_panel_step_hash")
        panel_fingerprint = step.get("_fixed_panel_fingerprint")
        panel_step_index = step.get("_fixed_panel_step_index", state_idx)
        ...
        for ego in egos:
            ...
            if crossfit is not None:
                ...
                for j in neighbor_ids:
                    prediction_samples.append({
                        ...
                        # Cross-fitted AIPW is deliberately forbidden from
                        # consuming downstream z/m/belief state.  Supply the
                        # same raw partition-independent ContextBlock contract
                        # used to fit the nuisance/effect models.
                        "context_block": step["proxy_context_blocks"][ego],
                        "neighbor_id": int(j),
                    })
            ...
            aggregate_row = {
                ...
                "ego_id": ego,
                "fixed_panel_step_index": int(panel_step_index),
                "fixed_panel_step_hash": panel_step_hash,
                "fixed_panel_fingerprint": panel_fingerprint,
                "top_k": top_k,
            ...
            for j in neighbor_ids:
                ...
                valid_actions_j = np.flatnonzero(valid_mask_j)
                learned_q_full_j = np.asarray(
                    learned_scores["q"][j], dtype=np.float64
                )
                learned_q_j = learned_q_full_j[valid_mask_j]
                oracle_q_raw_j = np.asarray(
                    oracle_scores["q"][j], dtype=np.float64
                )
                if oracle_q_raw_j.size == valid_mask_j.size:
                    oracle_q_j = oracle_q_raw_j[valid_mask_j]
                elif oracle_q_raw_j.size == valid_count_j:
                    oracle_q_j = oracle_q_raw_j
                else:
                    raise RuntimeError(
                        "Oracle response surface does not align with the valid action mask: "
                        f"oracle={oracle_q_raw_j.size}, valid={valid_count_j}, "
                        f"action_dim={valid_mask_j.size}"
                    )
                if valid_count_j >= 2:
                    ...
                    boundary = functional_boundary_diagnostics(
                        learned_q_j, oracle_q_j, action_ids=valid_actions_j
                    )
                    target_pi_valid = np.asarray(
                        target_pi_j[valid_mask_j], dtype=np.float64
                    )
                    target_pi_valid = target_pi_valid / np.clip(
                        target_pi_valid.sum(), 1e-12, None
                    )
                    contrast_weights = (
                        target_pi_valid - uniform_valid_j[valid_mask_j]
                    )
                    direction_boundary = direction_sign_boundary_diagnostics(
                        learned_q_j, oracle_q_j, contrast_weights
                    )
                    learned_q_range = float(np.ptp(learned_q_j))
                    oracle_q_range = float(np.ptp(oracle_q_j))
                    capacity_q_error_bound = float(
                        2.0 * boundary["q_gauge_sup_error"]
                    )
                    capacity_q_bound_violation = int(
                        abs(learned_q_range - oracle_q_range)
                        > capacity_q_error_bound + 1e-9
                    )
                else:
                    ...
                    boundary = {
                        ...
                        "oracle_argmin_action": -1,
                        "learned_argmax_action": -1,
                        "learned_argmin_action": -1,
                        "oracle_argmax_count": 0,
                        "oracle_argmin_count": 0,
                        "oracle_extrema_unique": 0,
                        "argmax_match": 0,
                        "argmin_match": 0,
                        "both_extrema_match": 0,
                        "oracle_g_plus": float("nan"),
                        "oracle_g_minus": float("nan"),
                        "oracle_extrema_gap": float("nan"),
                        "extrema_gap_zero": 1,
                        "q_raw_sup_error": float("nan"),
                        "q_gauge_shift_opt": float("nan"),
                        "q_gauge_sup_error": float("nan"),
                        "q_gauge_sup_error_check": float("nan"),
                        "lambda_C": float("inf"),
                        "lambda_C_raw": float("inf"),
                        "extrema_stability_certified": 0,
                        "extrema_stability_violation": 0,
                    }
                    direction_boundary = {
                        "learned_direction_from_q": float("nan"),
                        "oracle_direction_from_q": float("nan"),
                        "direction_weight_l1": float("nan"),
                        "direction_q_error_bound": float("nan"),
                        "direction_sign_margin": float("nan"),
                        "lambda_D": float("inf"),
                        "direction_sign_match_from_q": 0,
                        "direction_sign_certified": 0,
                        "direction_sign_violation": 0,
                    }
                    learned_q_range = float("nan")
                    oracle_q_range = float("nan")
                    capacity_q_error_bound = float("nan")
                    capacity_q_bound_violation = 0
                rows.append({
                    "seed": int(args.seed),
                    "state_idx": int(state_idx),
                    "ego_id": ego,
                    ...
                    "dr_applied": int(score_meta["dr_applied"]),
                    "learned_q_range": learned_q_range,
                    "oracle_q_range_from_surface": oracle_q_range,
                    "capacity_head_q_range_consistency_abs": float(abs(
                        learned_scores["capacity"][j] - learned_q_range
                    )) if np.isfinite(learned_q_range) else float("nan"),
                    "capacity_q_error_bound": capacity_q_error_bound,
                    "capacity_q_error_bound_violation": capacity_q_bound_violation,
                    "direction_head_q_consistency_abs": float(abs(
                        learned_scores["direction"][j]
                        - direction_boundary["learned_direction_from_q"]
                    )) if np.isfinite(float(direction_boundary["learned_direction_from_q"])) else float("nan"),
                    "oracle_direction_q_consistency_abs": float(abs(
                        oracle_scores["direction"][j]
                        - direction_boundary["oracle_direction_from_q"]
                    )) if np.isfinite(float(direction_boundary["oracle_direction_from_q"])) else float("nan"),
                    **boundary,
                    **direction_boundary,
                })


    # V7 two-level capacity geometry.  V6 established a within-relation phase
    # boundary (lambda_C) but the five-seed results falsified the hypothesis
    # that local extrema stability alone explains global C ranking.  Evaluate
    # the second selection layer explicitly through between-relation margins.
    aggregate_by_key = {
        (int(row["state_idx"]), int(row["ego_id"])): row
        for row in aggregate_rows
    }
    relation_groups = {}
    for row in rows:
        relation_groups.setdefault(
            (int(row["state_idx"]), int(row["ego_id"])), []
        ).append(row)

    ranking_diagnostic_count = 0
    ranking_q_certified_count = 0
    ranking_q_certified_violations = 0
    ranking_interval_certified_count = 0
    ranking_interval_certified_violations = 0
    ranking_q_bound_violations = 0
    ranking_pair_q_certified = 0
    ranking_pair_q_violations = 0
    lambda_topk_q_values = []
    topk_gap_values = []

    for key, selected_rows in relation_groups.items():
        selected_rows = sorted(selected_rows, key=lambda row: int(row["neighbor_id"]))
        aggregate = aggregate_by_key.get(key)
        if aggregate is None or len(selected_rows) < 2:
            continue
        q_errors = np.asarray([
            float(row.get("q_gauge_sup_error", float("nan")))
            for row in selected_rows
        ], dtype=np.float64)
        if not np.all(np.isfinite(q_errors)):
            continue
        diag = relation_ranking_diagnostics(
            [int(row["neighbor_id"]) for row in selected_rows],
            [float(row["learned_range"]) for row in selected_rows],
            [float(row["oracle_range"]) for row in selected_rows],
            q_errors,
            top_k=int(aggregate.get("top_k", 1)),
        )
        ranking_diagnostic_count += 1
        ranking_q_certified_count += int(diag["capacity_topk_q_certified"])
        ranking_q_certified_violations += int(
            diag["capacity_topk_q_certified_violation"]
        )
        ranking_interval_certified_count += int(
            diag["capacity_topk_interval_certified"]
        )
        ranking_interval_certified_violations += int(
            diag["capacity_topk_interval_certified_violation"]
        )
        ranking_q_bound_violations += int(
            diag["capacity_q_error_bound_violation_count"]
        )
        ranking_pair_q_certified += int(diag["q_certified_relation_pair_count"])
        ranking_pair_q_violations += int(
            diag["q_certified_relation_pair_violation_count"]
        )
        if np.isfinite(float(diag["lambda_topk_Q"])):
            lambda_topk_q_values.append(float(diag["lambda_topk_Q"]))
        if np.isfinite(float(diag["oracle_topk_gap"])):
            topk_gap_values.append(float(diag["oracle_topk_gap"]))

        per_relation = diag.pop("per_relation")
        aggregate.update({
            "oracle_topk_ids": "|".join(map(str, diag.pop("oracle_topk_ids"))),
            "learned_topk_ids": "|".join(map(str, diag.pop("learned_topk_ids"))),
            **diag,
        })
        for row in selected_rows:
            row.update(per_relation[int(row["neighbor_id"])])

    direction_certified_count = int(sum(
        int(row.get("direction_sign_certified", 0)) for row in rows
    ))
    direction_certified_violations = int(sum(
        int(row.get("direction_sign_violation", 0)) for row in rows
    ))
    capacity_q_bound_violation_count = int(sum(
        int(row.get("capacity_q_error_bound_violation", 0)) for row in rows
    ))
    ...
    policy_concentration_threshold = float(
        tiny_cfg.get("h1_min_policy_support_entropy", 0.50)
    )
    target_policy_concentrated_rows = [
        row for row in rows
        if float(row.get("policy_support_entropy", 1.0)) < policy_concentration_threshold
    ]
    target_policy_concentrated_pairs = {
        (int(row["ego_id"]), int(row["neighbor_id"]))
        for row in target_policy_concentrated_rows
    }
    replay_support = _pair_replay_support_diagnostics(
        runner.proxy.buffer,
        runner.action_dim,
        min_action_count=int(tiny_cfg.get("h1_min_pair_action_count", 2)),
        min_action_ess=float(tiny_cfg.get("h1_min_pair_action_ess", 2.0)),
        min_behaviour_propensity=float(
            tiny_cfg.get("h1_min_behaviour_propensity_support", 1e-6)
        ),
    )
    support_poor_pairs = replay_support["poor_pairs"]
    support_poor_rows = [
        row for row in rows
        if (int(row["ego_id"]), int(row["neighbor_id"])) in support_poor_pairs
    ]

    # Functional-boundary diagnostics for the extremal capacity C=max Q-min Q.
    # These are scientific diagnostics, not protocol-integrity gates.  The
    # gauge-invariant error delta_Q^circ is used because a constant shift of Q
    # changes neither C nor extrema identities.
    boundary_rows = [
        row for row in rows
        if np.isfinite(float(row.get("q_gauge_sup_error", float("nan"))))
    ]
    finite_lambda_rows = [
        row for row in boundary_rows
        if np.isfinite(float(row.get("lambda_C", float("inf"))))
    ]

    def _boundary_bin_summary(selected):
        if not selected:
            return {
                "n": 0,
                "extrema_swap_rate": float("nan"),
                "capacity_mae": float("nan"),
                "both_extrema_match_rate": float("nan"),
            }
        return {
            "n": int(len(selected)),
            "extrema_swap_rate": float(np.mean([
                1.0 - float(row.get("both_extrema_match", 0)) for row in selected
            ])),
            "capacity_mae": float(np.mean([
                float(row.get("capacity_abs_error", float("nan"))) for row in selected
            ])),
            "both_extrema_match_rate": float(np.mean([
                float(row.get("both_extrema_match", 0)) for row in selected
            ])),
        }

    boundary_bins = {
        "gap_zero_or_tied_extrema": _boundary_bin_summary([
            row for row in boundary_rows if int(row.get("extrema_gap_zero", 0)) == 1
        ]),
        "lambda_lt_0_25": _boundary_bin_summary([
            row for row in finite_lambda_rows if float(row["lambda_C"]) < 0.25
        ]),
        "lambda_0_25_to_0_5": _boundary_bin_summary([
            row for row in finite_lambda_rows if 0.25 <= float(row["lambda_C"]) < 0.5
        ]),
        "lambda_0_5_to_1": _boundary_bin_summary([
            row for row in finite_lambda_rows if 0.5 <= float(row["lambda_C"]) < 1.0
        ]),
        "lambda_ge_1": _boundary_bin_summary([
            row for row in finite_lambda_rows if float(row["lambda_C"]) >= 1.0
        ]),
    }
    if len(finite_lambda_rows) >= 3:
        lambda_capacity_corr = float(safe_spearman(
            np.asarray([float(row["lambda_C"]) for row in finite_lambda_rows]),
            np.asarray([float(row["capacity_abs_error"]) for row in finite_lambda_rows]),
        )[0])
    else:
        lambda_capacity_corr = float("nan")

    functional_boundary = {
        "protocol_version": "h1_extrema_gap_v1",
        "error_metric": "delta_Q_circ=0.5*osc(qhat-q)",
        "phase_parameter": "lambda_C=delta_Q_circ/oracle_extrema_gap",
        "row_count": int(len(boundary_rows)),
        "oracle_extrema_unique_count": int(sum(
            int(row.get("oracle_extrema_unique", 0)) for row in boundary_rows
        )),
        "extrema_gap_zero_count": int(sum(
            int(row.get("extrema_gap_zero", 0)) for row in boundary_rows
        )),
        "certified_extrema_stability_count": int(sum(
            int(row.get("extrema_stability_certified", 0)) for row in boundary_rows
        )),
        "certified_extrema_stability_violation_count": int(sum(
            int(row.get("extrema_stability_violation", 0)) for row in boundary_rows
        )),
        "both_extrema_match_rate": (
            float(np.mean([float(row.get("both_extrema_match", 0)) for row in boundary_rows]))
            if boundary_rows else float("nan")
        ),
        "finite_lambda_count": int(len(finite_lambda_rows)),
        "lambda_C_median": (
            float(np.median([float(row["lambda_C"]) for row in finite_lambda_rows]))
            if finite_lambda_rows else float("nan")
        ),
        "lambda_C_capacity_abs_error_spearman": lambda_capacity_corr,
        "bins": boundary_bins,
    }

    two_level_capacity = {
        "protocol_version": "h1_two_level_capacity_margin_v1",
        "interpretation": (
            "lambda_C controls within-relation extrema stability; lambda_topk_Q "
            "controls between-relation TopK stability using deterministic "
            "capacity error bounds 2*delta_Q_circ"
        ),
        "state_ego_count": int(ranking_diagnostic_count),
        "capacity_q_error_bound_violation_count": int(capacity_q_bound_violation_count),
        "topk_q_certified_count": int(ranking_q_certified_count),
        "topk_q_certified_violation_count": int(ranking_q_certified_violations),
        "topk_interval_certified_count": int(ranking_interval_certified_count),
        "topk_interval_certified_violation_count": int(ranking_interval_certified_violations),
        "q_certified_relation_pair_count": int(ranking_pair_q_certified),
        "q_certified_relation_pair_violation_count": int(ranking_pair_q_violations),
        "lambda_topk_Q_median": (
            float(np.median(lambda_topk_q_values))
            if lambda_topk_q_values else float("nan")
        ),
        "oracle_topk_gap_median": (
            float(np.median(topk_gap_values))
            if topk_gap_values else float("nan")
        ),
    }

    finite_lambda_d = [
        float(row["lambda_D"]) for row in rows
        if np.isfinite(float(row.get("lambda_D", float("inf"))))
    ]
    direction_sign_boundary = {
        "protocol_version": "h1_direction_sign_margin_v1",
        "certified_count": int(direction_certified_count),
        "certified_violation_count": int(direction_certified_violations),
        "lambda_D_finite_count": int(len(finite_lambda_d)),
        "lambda_D_median": (
            float(np.median(finite_lambda_d)) if finite_lambda_d else float("nan")
        ),
        "definition": "lambda_D=(||pi-q||_1*delta_Q_circ)/|D|",
    }
    ...
    metadata = {
        ...
        "h1_min_policy_support_entropy": policy_concentration_threshold,
        "target_policy_concentration_definition": "normalized_target_policy_entropy",
        "support_quality_definition": replay_support["support_scope"],
        "pair_replay_support": {
            key: value for key, value in replay_support.items() if key != "poor_pairs"
        },
        ...
        # Historical code reported dr_exercised=True when DR was not even
        # requested. Keep the field but give it literal semantics and expose
        # separate availability/request flags for fail-closed analysis.
        "dr_code_path_available": True,
        "dr_requested": bool(dr_requested),
        "dr_estimator_applied": bool(dr_requested and dr_rows > 0),
        "dr_exercised": bool(dr_requested and dr_rows > 0),
        "dr_integrity_gate_pass": bool((not dr_requested) or dr_rows > 0),
        ...
        "functional_boundary": functional_boundary,
        "functional_boundary_certified_extrema_count": int(
            functional_boundary["certified_extrema_stability_count"]
        ),
        "functional_boundary_violation_count": int(
            functional_boundary["certified_extrema_stability_violation_count"]
        ),
        "functional_boundary_lambda_capacity_error_spearman": float(
            functional_boundary["lambda_C_capacity_abs_error_spearman"]
        ),
        "two_level_capacity": two_level_capacity,
        "two_level_capacity_topk_q_certified_count": int(
            two_level_capacity["topk_q_certified_count"]
        ),
        "two_level_capacity_topk_q_violation_count": int(
            two_level_capacity["topk_q_certified_violation_count"]
        ),
        "two_level_capacity_q_bound_violation_count": int(
            two_level_capacity["capacity_q_error_bound_violation_count"]
        ),
        "direction_sign_boundary": direction_sign_boundary,
        "direction_sign_boundary_certified_count": int(
            direction_sign_boundary["certified_count"]
        ),
        "direction_sign_boundary_violation_count": int(
            direction_sign_boundary["certified_violation_count"]
        ),
        "q_centered_sq_error_sum": q_sq_error,
        "q_centered_oracle_sq_sum": q_sq_oracle,
        "q_centered_value_count": q_value_count,
        "support_poor_pair_count_mean": int(replay_support["poor_pair_count"]),
        "local_support_poor_pair_count_mean": int(replay_support["poor_pair_count"]),
        "target_policy_concentrated_pair_count_mean": int(len(target_policy_concentrated_pairs)),
        "target_policy_concentrated_pair_row_count_mean": int(len(target_policy_concentrated_rows)),
    ...
    metadata["alignment_protocol_gate_pass"] = bool(
        ...
        and metadata["dr_integrity_gate_pass"]
...
def run_tiny_task(args, cfg, device, out_dir=None, run_label="standalone"):
    ...
    for key in numeric_keys:
        ...
        summary.setdefault("metric_availability", {})[key] = (
            "available" if len(vals) > 0 else "unavailable"
        )
        if len(vals) > 0:
            ...
            # Unavailable/not-fitted is not scientific zero. Downstream
            # collectors already ignore None; keep missing metrics explicit.
