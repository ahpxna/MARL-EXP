...
# disappear (correct RC-3 warning below).
    ...
    def valid_action_mask(self, agent_id):
        """Command-level legality mask for clone-state interventions.

        OmniArena accepts every action id in ``[0, N_ACTIONS)`` for every
        agent at every nonterminal state.  Grid physics may make a movement
        attempt have no positional effect (for example because of a collision),
        but that is an outcome of an honored action, not an invalid request.
        Keeping legality separate from state-effect realization is important for
        support geometry: blocked movement must not be mislabeled as missing
        action support.
        """
        agent_id = int(agent_id)
        if agent_id < 0 or agent_id >= int(self.n_agents):
