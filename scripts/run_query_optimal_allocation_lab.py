...
PROTOCOL_VERSION='functional_matched_budget_allocation_v1'
STRATEGIES=(
 'uniform','behavior_freq','uncertainty_only','D_neyman_pilot','D_oracle',
 'C_extremal_confidence','C_successive_elimination','C_split_eval','C_oracle',
)
...
def _successive_counts(B,truth,sigma,obs):
    K=len(truth); counts=np.ones(K,dtype=int); z=2.0
    while int(counts.sum())<B:
        means=np.asarray([_mean(obs[a],counts[a]) for a in range(K)])
        rad=z*sigma/np.sqrt(counts)
        ...
        # sample most uncertain surviving extremal contender
...
def run(instances=200,seeds=(0,),budgets=(16,32,64,128,256),R=5,K=6,topk=2):
        ...
        for idx in range(int(instances)):
            ...
            for B in budgets:
                for strategy in STRATEGIES:
                    ...
                    for r in range(R):
                        if strategy=='C_successive_elimination': counts,meta=_successive_counts(B,truths[r],sigmas[r],obs[r])
    ...
    # Native winners by downstream endpoint; oracle strategies are diagnostics, not deployable methods.
    winners={}
    for B,by in summary.items():
        winners[B]={'C_mae':min(by,key=lambda s:by[s]['C_mae']),'C_topk_exact':max(by,key=lambda s:by[s]['C_topk_exact']),'D_mae':min(by,key=lambda s:by[s]['D_mae']),'D_sign_accuracy':max(by,key=lambda s:by[s]['D_sign_accuracy'])}
    return {'protocol_version':PROTOCOL_VERSION,'development_only':True,'instances_per_seed':int(instances),'seeds':list(map(int,seeds)),'budgets_per_relation':list(map(int,budgets)),'relations':int(R),'actions':int(K),'strategies':STRATEGIES,'matched_budget_contract':'every strategy uses exactly B samples per relation; common-random-number cell streams shared within instance','by_budget':summary,'endpoint_winners':winners}
