...
import numpy as np
from research_chains.experimental_extensions import d6_diagnostics,d6_sharpness_search,product_anova_decomposition,arbitrary_full_world
...
PROTOCOL_VERSION='d6_extension_lab_v1'
...
def run(instances=500,seed=0,sharpness_instances=5000):
    ...
    return {'protocol_version':PROTOCOL_VERSION,'development_only':True,'instances':int(instances),'seed':int(seed),'lean_alignment':{'hybrid_telescoping':True,'s1_exact_reproduction':{'surrogate_tightness':s1_exact['surrogate_tightness'],'delta_square':s1_exact['delta_square'],'surrogate_sup_error':s1_exact['surrogate_sup_error'],'residual_identity_worst_error':s1_exact['residual_identity_worst_error']}},'worst_candidate_violations':violations,'stress_by_family':by,'sharpness':d6_sharpness_search(instances=int(sharpness_instances),seed=seed+99),'decision_improvement_candidate':{'candidate_rhs':'(m-1)*delta_square/2','safe_fraction':float(np.mean([r['decision_candidate_half_violation']<=1e-10 for r in rows])),'worst_violation':float(max(r['decision_candidate_half_violation'] for r in rows)),'max_regret_over_original_approx_rhs':float(np.max(decision_ratio))},'qL1_distributional':{'surrogate_optimal_fraction':float(np.mean([r['qL1_topc_surrogate_optimal'] for r in rows])),'conditional_cases':len(ql1_rows),'conditional_worst_violation':float(max([r['qL1_conditional_violation'] for r in ql1_rows],default=0.0))},'assumption_rejection':rejection,'anova_comparison':anova_compare}

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument('--instances',type=int,default=500); p.add_argument('--sharpness-instances',type=int,default=5000); p.add_argument('--seed',type=int,default=0); p.add_argument('--out',default='research/high_value_extensions/d6/summary.json'); a=p.parse_args(argv)
    payload=run(a.instances,a.seed,a.sharpness_instances); atomic_json(Path(a.out),payload); print(json.dumps(payload,indent=2)); return 0 if payload['worst_candidate_violations']['worst']<=1e-8 and payload['worst_candidate_violations']['decision']<=1e-8 else 2
