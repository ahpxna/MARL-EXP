...
def _isolation_rows(
    ...
    # Full-Explicit is the selector-neutral fidelity reference even when the
    # caller requests only a subset of reported variants.  Keep it internal
    # rather than requiring users to include it in --variants.
    requested_variants = list(variants)
    run_variants = list(dict.fromkeys(requested_variants + ["Full-Explicit"]))
    records = {variant: [] for variant in run_variants}
...
def main(argv=None):
    ...
    if (
        ...
        or args.agent_count < 5
        or not args.seeds
    ):
        parser.error(
            "episodes, pretrain episodes, selector/challenge state budgets, "
            "core budget, agent count (>=5 for the OmniArena P2-role contract), "
            "and seeds must satisfy their positive confirmatory constraints"
        )
