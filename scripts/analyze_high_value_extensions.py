...
def _load(root,name):
    out=[]
    for p in sorted((Path(root)/name).glob('seed*.json')):
...
def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument('--root',default='research/high_value_extensions/runs'); p.add_argument('--out',default='research/high_value_extensions/DECISION_MATRIX.json'); p.add_argument('--min-promote-seeds',type=int,default=5); a=p.parse_args(argv); root=Path(a.root)
    ...
    if M:
        ...
        largest='256'; passive=_mean(M,lambda r:r['active_acquisition']['passive'][largest]['kernel_tv']['mean']); targeted=_mean(M,lambda r:r['active_acquisition']['targeted'][largest]['kernel_tv']['mean']); cal=_mean(M,lambda r:r['heldout_calibration']['targeted'][largest]['heldout_coverage']); safe=_mean(M,lambda r:r['cascade']['safe_rate']); fallback=_mean(M,lambda r:r['cascade']['fallback_rate'])
        ratio=targeted/passive if passive else None; enough=len(M)>=int(a.min_promote_seeds); deterministic_ok=ru<=1e-8 and mt<=1e-8; utility_nonvacuous=(fallback is not None and fallback<0.95); acquisition_gain=(ratio is not None and ratio<1.0); calibration_ok=(cal is not None and cal>=0.90); safe_ok=(safe is not None and safe>=1.0-1e-12)
        if not deterministic_ok: status='FIX'
        elif not enough: status='CONTINUE_MORE_SEEDS'
        elif not utility_nonvacuous: status='CONTINUE_VACUOUS'
        elif acquisition_gain and calibration_ok and safe_ok: status='PROMOTE'
        else: status='CONTINUE'
        decisions['MASTER']={'status':status,'evidence_seeds':len(M),'min_promote_seeds':int(a.min_promote_seeds),'promotion_eligible':bool(enough),'RU_worst_violation':ru,'MASTER_worst_violation':mt,'targeted_vs_passive_TV_ratio':ratio,'heldout_calibration_coverage':cal,'cascade_safe_rate':safe,'cascade_fallback_rate':fallback,'utility_nonvacuous':bool(utility_nonvacuous),'box_gamma_mean_tightening':_mean(M,lambda r:r.get('box_gamma',{}).get('mean_tightening')),'box_gamma_strict_fraction':_mean(M,lambda r:r.get('box_gamma',{}).get('strict_tightening_fraction')),'probability_lift_good_event_false_count':sum(r.get('probability_lift',{}).get('false_certificate_on_good_event_count',0) for r in M),'RU2_sharpness_reproduced':all(r.get('reference_uncertainty',{}).get('RU2_factor_two_sharpness',{}).get('sharpness_reproduced',False) for r in M),'next_gate':'external reference-identification + calibrated learned intervals'}
    if FD or FA or FS:
    ...
    if D:
        ...
        decisions['D6']={'status':'CONTINUE' if theorem<=1e-8 else 'FIX','evidence_seeds':len(D),'current_bound_worst_violation':theorem,'best_surrogate_sharpness_ratio':sur,'best_decision_sharpness_ratio':dec,'half_factor_candidate_seed_safe_fraction':half_safe,'qavg_uniform_seed_safe_fraction':qavg_uniform,'qavg_distributional_seed_safe_fraction':qavg_dist,'qL1_conditional_seed_safe_fraction':ql1_safe,'next_gate':'promote q-sensitive branch only if exact-hybrid quantities remain violation-free and materially tighter; decision half-factor remains candidate until formal proof'}
    if Q:
        native=[]
        for r in Q:
        ...
        decisions['QUERY']={'status':'CONTINUE','native_advantage_fraction':float(np.mean(native)) if native else None,'representation_conflict_mean':conflict,'next_gate':'replace synthetic proxies with real MARL/XAI baselines and external challenge environments'}
    if S:
