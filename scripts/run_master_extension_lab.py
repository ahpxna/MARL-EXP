...
PROTOCOL_VERSION="master_extension_lab_v1"
...
def run(instances=100,budgets=(16,32,64,128,256),seed=0,tolerance=0.05):
    ...
    modes=("passive","randomized","targeted")
    ...
    for idx in range(int(instances)):
        ...
        truth=set(world.support.omega); full=set(world.support.full_product()); lower_items=[a for a in truth if rng.random()<0.7]
        if not lower_items: lower_items=[next(iter(truth))]
        upper_items=tuple(sorted(truth|{a for a in full-truth if rng.random()<0.2}))
        bracket=SupportBracket(SupportModel(world.support.action_sizes,tuple(sorted(lower_items)),key="lower"),SupportModel(world.support.action_sizes,upper_items,key="upper"))
        ...
        support_records.append({"mean_width":float(np.mean(cplus-cminus)),"max_width":float(np.max(cplus-cminus)),"truth_covered":bool(np.all(cminus<=primitive_scores+1e-10) and np.all(primitive_scores<=cplus+1e-10))})
    def agg(rows,key):
    ...
    return {
        ...
        "support_uncertainty":{"coverage_rate":float(np.mean([r['truth_covered'] for r in support_records])),"mean_span_interval_width":float(np.mean([r['mean_width'] for r in support_records]))},
        "cascade":{"fallback_rate":float(np.mean([r['fallback'] for r in cascade])),"safe_rate":float(np.mean([r['certificate_safe'] for r in cascade])),"mean_regret":float(np.mean([r['regret'] for r in cascade])),"mean_exact_runtime_when_called":float(np.mean([r['exact_runtime_seconds'] for r in cascade if r['fallback']])) if any(r['fallback'] for r in cascade) else 0.0,"mean_cascade_runtime":float(np.mean([r['cascade_runtime_seconds'] for r in cascade]))},
    }
