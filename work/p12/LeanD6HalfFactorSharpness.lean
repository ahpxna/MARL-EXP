  ...
  have hmax : maxVal r = 1 := by
    ...
    · have h := le_maxVal r (action3 0 1 0)
      have hw : r (action3 0 1 0) = 1 := by
        simp only [r, Finset.sum_singleton]
        change 2 - productResponse pointMassAnchor3 sharpWorld3 2 0 = 1
        rw [sharp_response_zero]
        simp <;> norm_num
      rw [hw] at h
      exact h
  have hmin : minVal r = -2 := by
    ...
    · have h := minVal_le r (action3 0 0 1)
      have hw : r (action3 0 0 1) = -2 := by
        simp only [r, Finset.sum_singleton]
        change 0 - productResponse pointMassAnchor3 sharpWorld3 2 1 = -2
        rw [sharp_response_one]
        simp <;> norm_num
      rw [hw] at h
      exact h
    · apply le_minVal
